...
pub const Player = struct {
    ...
    pub fn facing_tile(self: *const Player) [2]u8 {
        ...
        return self.walker.facing_tile(map.MAP_W, map.MAP_H);
    }

    /// Pixel x for sprite rendering. Centers 32px sprite on 16px tile.
    pub fn screen_x(self: *const Player) i32 {
        return self.walker.pixel_x() - 8;
    }

    /// Pixel y for sprite rendering. Aligns 32px sprite bottom to tile bottom.
    pub fn screen_y(self: *const Player) i32 {
        return self.walker.pixel_y() - 16;
    }
